...
echo >> cron_mod.conf
echo TOPIC_LAMBDA=$(cat ./iot_prov_config | grep TOPIC_LAMBDA | awk -F'=' '{print $2}') >> cron_mod.conf
echo TOPIC_DASHBOARD=$(cat ./iot_prov_config | grep TOPIC_DASHBOARD | awk -F'=' '{print $2}') >> cron_mod.conf
echo >> cron_mod.conf
...
if [ ! -e /home/pi/motion_detect.py ]; then
    ...
    echo @reboot python /home/pi/rain_detect.py >> cron_mod.conf
    fi
...
echo >> ../.profile
echo export TOPIC_LAMBDA=$(cat ./iot_prov_config | grep TOPIC_LAMBDA | awk -F'=' '{print $2}') >> ../.profile
echo export TOPIC_DASHBOARD=$(cat ./iot_prov_config | grep TOPIC_DASHBOARD | awk -F'=' '{print $2}') >> ../.profile
echo >> ../.profile
